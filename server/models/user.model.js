...
UserSchema.path('username').validate(function (value, respond) {
  ...
  return this.constructor.findOne({ username: value }).exec().then(function (user) {
    if (user) {
      if (self.id === user.id) {
        return respond(true);
      }
...
UserSchema.methods = {
  ...
  encryptPassword: function encryptPassword(password, callback) {
    ...
    if (!callback) {
      return _crypto2.default.pbkdf2Sync(password, salt, defaultIterations, defaultKeyLength, digest).toString('base64');
    }

    return _crypto2.default.pbkdf2(password, salt, defaultIterations, defaultKeyLength, digest, function (err, key) {
      if (err) {
        callback(err);
      } else {
